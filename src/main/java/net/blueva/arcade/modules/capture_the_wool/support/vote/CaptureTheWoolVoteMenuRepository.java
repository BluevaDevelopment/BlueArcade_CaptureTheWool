...
public class CaptureTheWoolVoteMenuRepository {

    private final ModuleConfigAPI moduleConfig;
    private final Map<String, MenuDefinition<Material>> menus = new HashMap<>();

    public CaptureTheWoolVoteMenuRepository(ModuleConfigAPI moduleConfig) {
        this.moduleConfig = moduleConfig;
    }

    public void loadMenus() {
        menus.clear();
        loadMenu(CaptureTheWoolVoteService.MENU_MAIN, "menus/java/capture_the_wool_vote_main.yml",
                "menus/bedrock/capture_the_wool_vote_main.yml");
        loadMenu(CaptureTheWoolVoteService.MENU_HEARTS, "menus/java/capture_the_wool_vote_hearts.yml",
                "menus/bedrock/capture_the_wool_vote_hearts.yml");
        loadMenu(CaptureTheWoolVoteService.MENU_TIME, "menus/java/capture_the_wool_vote_time.yml",
                "menus/bedrock/capture_the_wool_vote_time.yml");
        loadMenu(CaptureTheWoolVoteService.MENU_WEATHER, "menus/java/capture_the_wool_vote_weather.yml",
                "menus/bedrock/capture_the_wool_vote_weather.yml");
    }
    ...
    private void loadMenu(String id, String javaFile, String bedrockFile) {
        ...
        String title = javaConfig.getString("menu_name", javaConfig.getString("menuName", ""));
        int size = javaConfig.getInt("menu_size", javaConfig.getInt("menuSize", 9));
    ...
    private List<JavaMenuItem<Material>> loadJavaItems(ConfigurationSection section) {
        ...
        for (String key : section.getKeys(false)) {
            ...
            if (stackSection == null) {
                stackSection = itemSection.getConfigurationSection("itemStack");
            }
            if (stackSection == null) {
                continue;
            }
    ...
    private BedrockMenuDefinition loadBedrockMenu(String bedrockFile) {
        ...
        String title = bedrockConfig.getString("menu_name", bedrockConfig.getString("menuName", ""));
